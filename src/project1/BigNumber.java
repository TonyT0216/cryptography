...
public class BigNumber {
     ...
     * @param two    the second BigNumber
...
//			s.negate();
     ...
     * @param y
     * @returns:
     * 0 if the BigNumbers are the same
     * -1 if the passed in number is smaller
     * 1 if the passed in number is larger
    ...
    int compareTo(BigNumber y) {
        ...
        if (digits.size() == y.digits.size()) {
            for (int i = 0; i < digits.size(); i++) {
                if (digits.get(i).compareTo(y.digits.get(i)) == 0); else {
    ...
    public void negate() {
        ...
        for (int i = digits.size() - 1; i >= 0; i--) {
            if (Integer.parseInt(digits.get(i)) > 0) {
                doneZeros = true;
            }
            if (!doneFirst && doneZeros) {
                int temp = 10 - Integer.parseInt(digits.get(i));
                digits.set(i, "" + temp);
                //System.out.println("made it");
                doneFirst = true;
            } else if (doneFirst && doneZeros) {
                int temp = 9 - Integer.parseInt(digits.get(i));
                digits.set(i, "" + temp);
            }
        }
    }
    ...
    public void normalize() {
        ...
        for (int j = 0; j <= digits.size() + 1; j++) {
            if (Integer.parseInt(digits.get(0)) == 0) {
                digits.remove(0);
            }
        }
    }
